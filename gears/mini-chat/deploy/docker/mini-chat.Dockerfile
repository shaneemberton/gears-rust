...
# Build the cf-gears-server binary.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,sharing=locked \
    ...
    if [ -n "$CARGO_FEATURES" ]; then \
        cargo build $RELEASE_FLAG --bin cf-gears-server --package=cf-gears-server --features "$CARGO_FEATURES"; \
    else \
        cargo build $RELEASE_FLAG --bin cf-gears-server --package=cf-gears-server; \
    fi && \
