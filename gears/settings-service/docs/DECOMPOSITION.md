...
**Two things to settle with the DESIGN owners**, neither of which blocks a wave:
...
- **Scope**:
  ...
  - Gear scaffold: `#[toolkit::gear]` annotated gear with ClientHub registration for the SDK client traits, resolving the types registry, the authorization resolver, and the tenant resolver at init — the last to learn the root tenant's id once, since platform scope is that id (DESIGN §4.1, §4.7)
...
- **Scope**:
  - The `audit_records` table (DESIGN §4.7): `resource`, `declaration_key`, `tenant_id` (never `NULL`), `operation` in `create`, `change`, `revert`, `remove`, `clone`, `secret_use`, `actor` with its own `actor_classification`, masked `pre_value`/`post_value`, `outcome`, `request_id`, nullable `change_set_id`, `occurred_at`, `retain_until`; append-only, no `UPDATE` and no `DELETE` outside retention pruning
...
- **Depends On**: `cpt-cf-settings-service-feature-typed-value-validation`, `cpt-cf-settings-service-feature-audit-store`

- **Scope**:
  - Reconciler operations `register_declarations`, `retire_declarations`, and `list_contributed` behind the SDK Contribution client trait (DESIGN §4.2 *Module Contribution Reconciler*, §4.5)
  ...
  - Publication of `event_declaration_registered`, `event_declaration_retired`, and `event_declaration_reactivated`
...
- `cpt-cf-settings-service-feature-module-contributions` requires typed value validation, for the defaults and the copied values an upgrade re-validates, and the audit store, since registration, retirement and reactivation are audited mutations. It does not require value writes: the upgrade migration copies rows directly rather than setting them.

**Parallelism**: wave 1 is strictly sequential. In wave 2, `audit-store` can start as soon as 2.3 lands and run alongside 2.4 and 2.5; `module-contributions` needs only 2.4 and 2.6 and can run alongside 2.7 through 2.9; `tenant-access`, `value-writes`, and `secret-values` form a chain. Work also parallelizes inside 2.1, where the SDK crate, persistence harness, REST and OData infrastructure, error mapping, and the Audit Emitter are largely independent of one another.
