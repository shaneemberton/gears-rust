...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `apply_id` | UUID | Yes | The apply this record belongs to. `(apply_id, subscriber, key)` is the record's **unique identity** — `tenant` is **not** in the key (it is determined by the apply). It is **not** an ordering key: `apply_id` is a UUIDv7 minted when the apply is **created** (DESIGN.md §4.1 `ApplyOperation`), and because an apply commits **per change**, a long apply created earlier can settle later — so `apply_id` order is creation order, not settle order. "Older / newer" within a `(subscriber, key, tenant)` supersession group is therefore a comparison of **`apply_bundle_tracker.settle_seq`** (§4.7), the monotonic sequence assigned when the apply settles. |
...
| `settle_seq` | BIGSERIAL | No | auto | **Monotonic settle order** — the ordering key for supersession (§4.1, §4.2 *Apply Outcome Tracker*). Assigned in the bookkeeping transaction, i.e. after the apply has settled |
...
**Indexes:** unique `uq_tracker_apply` on `apply_id`; unique `uq_tracker_settle_seq` on `settle_seq`.

**Why a sequence and not `apply_id`.** `apply_id` is minted when the apply is **created** (DESIGN.md §4.1 `ApplyOperation`, §4.3 `POST /v1/applies`), so its order is creation order. An apply commits **per change** (§4.2 *Apply Publisher*), so a long apply created earlier can settle after a short one created later — comparing `apply_id` would then call the last-settled apply the older one and resolve it `superseded`, a terminal non-failure, while its value is the one actually stored. UUIDv7 is also only k-sortable to the millisecond and not strictly monotonic within a millisecond or across processes, so neither its value nor its embedded timestamp can carry this. `settle_seq` is assigned in the bookkeeping transaction that runs once **every** value of the apply has committed; because that apply commits per change there is no single "the apply committed" instant, so this is the closest available proxy for completion order — and unlike `apply_id` it is derived from completion rather than from start.
...
| Supersession ordering — settle, not creation | repo | An apply **created earlier but settled later** is the newer one: seed a long apply A (lower `apply_id`, higher `settle_seq`) and a short apply B, confirm B, and assert A is **not** superseded — ordering follows `settle_seq`, never `apply_id` |
...
Concurrent applies and concurrent back-responses exist; the policy is **guards rather than locks** (§4.2 *Apply Outcome Tracker*).

1. **Operations needing protection:** concurrent back-responses within one `(subscriber, key, tenant)` group; a bookkeeping transaction publishing a new apply for that group while a supersession scan is running; duplicate delivery of a single response.
2. **Policy:** every status transition is guarded by `WHERE status = 'awaiting'`, so a concurrently-terminal record is skipped rather than overwritten; `uq_await_record` enforces at-most-one row per `(apply_id, subscriber, key)`; `uq_tracker_apply` arbitrates competing tracker inserts, the loser treating the conflict as already-published. A record stranded by a late-committing bookkeeping transaction is repaired by its own next response, not prevented.
3. **Test pattern:** seed N applies, spawn back-responses with a barrier-synchronized start, and assert the **invariants** rather than one fixed outcome — every record ends terminal, no record leaves a terminal state once it reaches one, the on-demand counts equal the record statuses, and no `failed` record is ever masked by a `superseded` one. The split between `succeeded` and `superseded` may legitimately differ with arrival order, so pinning one exact `overall_status` would encode a race rather than a requirement.
4. **Publication:** under concurrent applies each apply's `cache_invalidate` is published once and its `apply_notification`s are delivered at-least-once (each delivery re-applied and re-acked by the consumer).
