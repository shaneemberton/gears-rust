...
| `cpt-cf-settings-service-fr-standard-advanced-mode` | `mode` on the declaration, carried on every declaration and browse item as a tag; no server-side filtering — a client groups locally; the preference lives with the console (§4.3) |
...
C4Context
 ...
 Rel(settings_service, idp, "validate step-up token (via the AuthN resolver)", "ClientHub")
...
C4Container
 ...
 Container_Ext(idp, "IdP / AuthN Resolver", "ClientHub", "Step-up token validation through the platform's AuthN resolver; the IdP is not called per write")
 ...
 Rel(domain, idp, "validate step-up token (via the AuthN resolver)", "ClientHub")
...
- **Standard / Advanced mode** — a per-declaration tag, `standard` or `advanced`, carried on every read so a client groups locally; this service filters nothing on it (§4.1, §4.3).
...
- **signature** — the AuthN resolver authenticates the token (`AuthNResolverClient::authenticate`); a token it refuses is refused here;
...
- Returns `value`, `source`, `source_scope`, the `fallback` with its `fallback_source` and `fallback_scope` (the *Fields* table above — one read answers both columns of the settings table, and the parent scope need not be readable by the caller for its value to appear as the fallback), resolved `traits`, `inheritance_trail`, `last_change_at`, and — when the **scope's own override** is flagged `needs_review` (resolution falls through past it, §4.2 *Value Resolver*, but the admin must still see it) — `needs_review` + `needs_review_detail` (§4.7), so the admin sees both the effective (fallthrough) value and that the own override needs fixing; the value type is `value_type_id` (§1.3) for both authors (`cpt-cf-settings-service-fr-typed-value-validation`, `cpt-cf-settings-service-fr-cascading-inheritance`).
...
| Method | Endpoint | Description | Idempotency |
|--------|----------|-------------|-------------|
| `GET` | `/settings-service/v1/search?q={query}&tenant={tenant_id}` | Cross-field search (flat list, breadcrumbs, matched-field); every hit carries its declaration's `mode` | Yes |
