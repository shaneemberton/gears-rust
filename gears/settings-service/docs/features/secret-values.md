...
3. [x] - `p1` - Build the store context with a fixed settings-service principal as subject, no subject type, the target tenant as subject tenant, and the first-party wildcard token scope, so the entry lives in the tenant the value belongs to, only this gear's principal reads it back, and the platform's token-scope enforcer — which fail-closes on an empty scope set before authorization is consulted at all — does not refuse the call while the store is neither down nor mis-provisioned; the scope widens nothing, since this gear's role assignment stays the authoritative limit - `inst-sv-ref-3`
