...
//! have no direct instances and define the `x-gts-traits-schema` (the
//! trait shape, with per-trait `default`s). Concrete derived vendor
//! schemas override / inherit the trait values. The envelopes also
//! declare their own `x-gts-traits` (equal to the trait-schema
//! defaults): gts-rust 0.10.0's entity-completeness check
//! (`validate_entity` -> `validate_entity_traits`) requires every
//! registered schema that carries a `x-gts-traits-schema` to also carry
//! `x-gts-traits` values, and — unlike `validate_schema_traits` — it
//! does NOT exempt abstract types. Derived schemas RFC 7396-merge over
//! these base defaults.
//!
//! Declared with `#[toolkit_gts::gts_type_schema(..)]` using the
...
#[gts_type_schema(
    ...
    traits = TenantTypeTraits { allowed_parent_types: Vec::new(), idp_provisioning: false },
    gts_abstract = true
)]
...
#[gts_type_schema(
    ...
    traits = TenantMetadataTraits { inheritance_policy: MetadataInheritancePolicy::OverrideOnly },
    gts_abstract = true
)]
