...
Constructor Studio kit script: lives in studio-kit-gears/scripts/pr.py.
        ...
        if os.path.isfile(agents):
            try:
                with open(agents, encoding="utf-8") as f:
                    head = f.read(512)
                if _CF_MARKER in head or _CPT_MARKER in head:
                    return current
            except OSError:
                pass
...
def _read_studio_path(project_root):
    """Read cf-studio-path variable from AGENTS.md TOML block.

    Parses the ```toml block inside the @cf:root-agents section.
    Returns the value (for example '.cf-studio') or None.
    """
    ...
    try:
        ...
        return None
    if _CF_MARKER not in content:
        return None
    # Extract cf-studio-path from TOML block.
    m = re.search(
        r'cf-studio-path\s*=\s*"([^"]+)"', content,
    )
    if not m:
        m = re.search(
            r"cf-studio-path\s*=\s*'([^']+)'", content,
...
if _STUDIO_REL is None:
    for _candidate in (".cf-studio", ".cypilot", "cypilot", "cpt"):
        if os.path.isdir(os.path.join(ROOT, _candidate)):
            ...
            break
    if _STUDIO_REL is None:
        _STUDIO_REL = ".cf-studio"

STUDIO_PATH = os.path.join(ROOT, _STUDIO_REL)


def _load_pr_config():
    """Load PR review config from {cf-studio-path}/config/pr-review.toml.
