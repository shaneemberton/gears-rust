...
cf: true
type: workflow
name: cf-gears-migrate-openspec
...
- [ ] Agent has access to Constructor Studio kit resources (resolved via `cypilot resolve-vars`)
...
1. Run `cypilot info` to get `{cf-studio-path}` and project root
...
```bash
python3 {cf-studio-path}/.core/skills/cypilot/scripts/cypilot.py validate
```
...
```bash
python3 {cf-studio-path}/.core/skills/cypilot/scripts/cypilot.py spec-coverage
```
...
```bash
python3 {cf-studio-path}/.core/skills/cypilot/scripts/cypilot.py toc {artifacts_output}/PRD.md {artifacts_output}/DESIGN.md {artifacts_output}/DECOMPOSITION.md
```
