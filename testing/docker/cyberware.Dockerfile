...
# Build the cf-gears-server binary in release mode
# Using --bin to build only the specific binary
# Features can be customized via CARGO_FEATURES build arg
RUN if [ -n "$CARGO_FEATURES" ]; then \
        cargo build --release --bin cf-gears-server --package=cf-gears-server --features "$CARGO_FEATURES"; \
    else \
        cargo build --release --bin cf-gears-server --package=cf-gears-server; \
    fi
...
COPY --from=builder /build/target/release/cf-gears-example-server /app/cf-gears-example-server
...
USER 1000
CMD ["/app/cf-gears-example-server", "--config", "/app/config/e2e-local.yaml"]
