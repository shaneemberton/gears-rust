...
class PrereqHSSrvMock(Prereq):
    def __init__(self):
        super().__init__(
            name="Gears example server is running with mock mode enabled",
            remediation=(
                "Start the Gears example server with mock mode enabled "
                "(use -mock option)"
            )
        )
    ...
    def check(self) -> bool:
        ...
        while time.time() - start_time < max_retry_time:
            try:
                ...
                if response.status_code != 200:
                    logging.debug(
                        f"Gears example server responded with status code: "
                    ...
                    try:
                        ...
                        for service in services:
                            if service.get('name') == 'mock':
                                ...
                                logging.info(
                                    "Gears example server is running with "
                                    "mock mode enabled"
                                )
                                break
                        ...
                        if mock_service_found:
                            ...
                            logging.debug(
                                "Gears example server is running but mock mode "
                ...
                logging.debug(
                    f"Connection error to Gears example server: {e} "
                    f"(attempt {attempt + 1})"
                )
                status = PRECHECK_WARNING
            except Exception as e:
                logging.debug(
                    f"Error checking Gears example server: {e} "
                    f"(attempt {attempt + 1})"
                )
                status = PRECHECK_WARNING
        ...
        if status == PRECHECK_WARNING:
            if attempt > 0:
                logging.warning(
                    f"Cannot connect to Gears example server at {BASE_URL} "
            ...
            logging.error(
                f"Gears example server communication failed after "
